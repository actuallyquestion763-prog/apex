...
export class CmsController {
  ...
  // Streams the file through the storage abstraction — never exposes the
  // storage bucket directly, so a request can only ever resolve to an
  // object key this service itself looked up from the CmsMedia table (see
  // CmsService.getMediaFile). See that method's comment for why "any valid
  // media id is servable" is the intended trust boundary here, not a
  // shortcut.
  @Get('media/:id')
